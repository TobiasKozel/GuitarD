...
int main() {
  ...
  for (int i = 0; i < channels; i++) {
    in[i] = new guitard::sample[512];
    out[i] = new guitard::sample[512];
    memset(in[i],0, 512 * sizeof(guitard::sample));
  }
  ...
  for (auto i : sizes) {
    ...
    size_t samplesLeft = samplesLeftTotal;
    while (samplesLeft > 0) {
    ...
    std::cout << i << "\t" << ms.count() << "\n";
    // std::cout << "Buffersize: " << i << "\n";
    // std::cout << "Waited " << total.count() << " ns\n";
    // std::cout << "Waited " << ms.count() << " ms\n";
  }
  return 0;
}